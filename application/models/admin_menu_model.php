...
class Admin_menu_model extends CI_Model{
  ...
  function generateallTree(&$tree, $parentid = 0) {
    ...
    function generateallActiveTreeArrayByUser($userid) {
        ...
        $sql = "select id,name,shortdesc,status,parentid,page_uri,`orderr`, adminmenu.resourceid, acluser.RESOURCEID from adminmenu left join (SELECT acl.RESOURCEID FROM usersgroups ug inner join accesscontrollist acl on ug.GROUPID = acl.TARGETID and acl.TYPEID = 1 inner join resources r on r.ID = acl.RESOURCEID inner join groups g on g.ID = ug.GROUPID where ug.USERID = ? and g.ENABLE = 1 UNION SELECT acl.RESOURCEID from accesscontrollist acl inner join resources r on r.ID = acl.RESOURCEID inner join users u on u.ID = acl.TARGETID where acl.TARGETID = ? and acl.TYPEID = 2 and u.ENABLE = 1 ) acluser on acluser.RESOURCEID = adminmenu.resourceid where status = 'active' and (acluser.resourceid is not null or adminmenu.resourceid is null) order by parentid asc, `order` asc";
    ...
    function generateTree1(&$tree, $parentid = 0) {
        ...
        $this->db->join('omc_page', 'adminmenu.id = omc_page.menu_id');
        $this->db->where ('parentid',$parentid);
  ...
  function getMenu($id){
      ...
      $Q = $this->db->where($options,1)->get('adminmenu');
      if ($Q->num_rows() > 0){
        $data = $Q->row_array();
      }
  ...
  function getrootMenus(){
   ...
   function addMenu($name, $shortdesc, $status, $parentid, $order, $page_uri, $resourceId=null){
        ...
        $this->db->insert('adminmenu', $data);
        return $this->identity();
   }
 
   function updateMenu($id, $name, $shortdesc, $status, $parentid, $order, $page_uri,$resourceId=null){
        $data = array(
            'name' =>  db_clean($name),
   ...
   function exportCsv(){
      $this->load->dbutil();
      $Q = $this->db->query("select * from adminmenu");
