...
/**
* Authentication and Access Control List Library
* @package Libraries
...
class Auth_acl
{
     ...
     */
    function login($nick, $password, $id_cicle, $description_cicle){
        ...
        if (strcmp($nick,$su) == 0 and strcmp($password,$pwd) == 0){
            $this->CI->session->set_userdata(array('logged_in' => true, 'user' => $su, 'id' => $suid, 'username' => $su, 'id_cicle' => $id_cicle, 'description_cicle'=>$description_cicle));
            $this->logged_date();
            return TRUE;
        }
        ...
        if($query->num_rows()>0){
            ...
            if (strcmp($dbpassword, $password) == 0){
                $fullname = $query->row()->NAME . ' ' . $query->row()->SURNAME;
                $this->CI->session->set_userdata(array('logged_in' => true, 'user' => $nick, 'id' => $query->row()->ID, 'username' => $fullname, 'id_cicle' => $id_cicle, 'description_cicle'=>$description_cicle));
                $data = TRUE;
            }
        }

        if ($data) $this->logged_date();
