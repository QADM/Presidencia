...
(function ($) {
        ...
        return this.each( function() {
            ...
            $("th",thr).each(function ( j ) {
                ...
                if(typeof ts.p.colModel[j].resizable === 'undefined') {
                    ...
                    // Y@SMANY Y @YLIENN de true para false. Resizable columns
    ...
    $.jgrid.extend({
        ...
        /**
    * @description Obtiene los records del grid
    ...
    **/
        ...
        /**
    * @description Obtiene los elementos seleccionados .
    * @author Alien Gongora Rodríguez
    * @return {Array} Arreglos con los objetos de los elementos seleccionados.
    **/
        getIdsRowsSelected:function(){
            var t = this[0];
            if(t.p.selarrrow == undefined || t.p.selarrrow.length == 0 ) return [];
            var _id = t.p.jsonReader.id == undefined ? 'id' : t.p.jsonReader.id;
            var objects = Array();
            for(var a = 0; a <t.p.selarrrow.length; a++ ){
                var object = new Object();
                object[_id] = t.p.selarrrow[a];
                objects.push(object);
            }
            return objects;
        },
        ...
        /**
    * @description Obtiene la cantidad de elementos seleccionados .
    * @author Alien Gongora Rodríguez
    * @return {Integer} Número de elementos seleccionados.
    **/
        getNumberRowSelected:function(){
            var t = this[0];
            if(t.p.selarrrow == undefined || t.p.selarrrow.length == 0)
                return 0;
            return  t.p.selarrrow.length;
        },
        /**
    * @description Retorna si hay elementos seleccionados.
    * @author Alien Gongora Rodríguez
    * @return {Array} True o False.
    **/
        existDataSelected:function(){
            var t = this[0];
            if(t.p.selarrrow == undefined || t.p.selarrrow.length == 0 ) return false;
            return true;
        },

        /**
    * @description Id del último elemento seleccionado.
    * @author Alien Gongora Rodríguez
    * @return {Object} Si no hay rows seleccionads retorna undefined.
    **/
        lastRowSelectedId:function(){
            var t = this[0];
            if( t.p.selrow == null || t.p.selrow == undefined )return undefined;
            return t.p.selrow
        },

        /**
    * @description Retorna el numero de la pagina activa
    * @author Alien Gongora Rodríguez
    * @return {Object} Retorna la pagina activa.
    **/
        numberPageActive: function(){
            var t = this[0];
            return t.p.page;
        },

        /**
    * @description Id Nombre del campo id del Grid.
    * @author Alien Gongora Rodríguez
    * @return {Object} Retorna el Id del grid.
    **/
        getIdRowGrid:function(){
            var t = this[0];
            return t.p.jsonReader.id == undefined ? {} : t.p.jsonReader.id;
        },

        /**
    * @description Selecciona una o un grupo de rows (Nota si esta cargando el grid,
    * esperar a que termine de cargar... o poner la propiedad async = false...)
    * @author Alien Gongora Rodríguez
    * @param  Array || Object - Array de Object si es multiselect.
    **/
        selectRows:function(/*Array || Object*/data){
            var t = this[0];
            var _id = t.p.jsonReader.id == undefined ? 'id' : t.p.jsonReader.id;
            if(data === undefined )
                return;
            if(!t.p.multiselect){
                $(t).setSelection(data[_id].toString(),true);
            }
            else{
                for(var a = 0; a < data.length; a++ )
                {
                    var _data = data[a];
                    $(t).setSelection(_data[_id].toString(),true);
                }
            }
        },
